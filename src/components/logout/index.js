import React, { useState } from 'react'
import styled from 'styled-components'
import { motion } from 'framer-motion';

const Logout = () => {
  const [hover, setHover] = useState(false)

  return (
    <Container onHoverStart={() => { setHover(true) }} onHoverEnd={() => { setHover(false) }}>
      <svg width="27" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path d="M9 21H5C4.46957 21 3.96086 20.7893 3.58579 20.4142C3.21071 20.0391 3 19.5304 3 19V5C3 4.46957 3.21071 3.96086 3.58579 3.58579C3.96086 3.21071 4.46957 3 5 3H9" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
        <StyledPath animate={hover ? { x: 3 } : { x: 0 }} d="M16 17L21 12L16 7" />
        <StyledPath animate={hover ? { x: 3 } : { x: 0 }} d="M21 12H9" />
      </svg>
      <Text>Logout</Text>
    </Container>
  );
}

const Container = styled(motion.button)`
  display: flex;
  align-items; center;
  font-family: 'Roboto', 'Arial', 'Helvetica';
  border: none;
  background: none;
  cursor: pointer;
`;

const StyledPath = styled(motion.path)`
  stroke: black;
  stroke-width: 2px;
  stroke-linecap: black;
  stroke-line-join: round;
`;

const Text = styled.span`
  height: 1.5rem;
  font-size: 1.25rem;
  margin-left: .5rem;
`;
export default Logout;